...
fn intersect(tri: &Triangle, plane_z: f64) -> Option<Line2d> {
    ...
    if plane_pts.len() == 1 {
        // it's either completely outside and just touching (return none, no intersect)
        // or inside with a point on the plane (manually get intersections and continue)
        let remaining: Vec<&Point3d> = tri.pts.iter().filter(|p3d| p3d.z != plane_z).collect();
        if between(remaining[0].z, remaining[1].z, plane_z) {
            intersections.push(*plane_pts[0]);
            intersections.push(zlerp(&Line3d(*remaining[0], *remaining[1]), plane_z));
        } else {
            return None;
        }
